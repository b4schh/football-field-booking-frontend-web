...
export default function PaymentUpload() {
  ...
  const { fetchBookingById, uploadPaymentProof, isLoading } = useBookingStore();
  ...
  const loadBookingDetails = async () => {
    ...
    if (result.success) {
      ...
      // Check if booking is not Pending (already uploaded or other status)
      if (bookingData.bookingStatus !== 0) {
        toast.info("Đơn đặt sân này đã được xử lý.");
        setTimeout(() => {
      ...
      const holdExpiresAt = new Date(bookingData.holdExpiresAt);
      const now = new Date();
  ...
  const handleUpload = async () => {
    ...
    const result = await uploadPaymentProof(id, formData);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-6 md:px-8 lg:px-20">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">Thanh toán tiền cọc</h1>
          <p className="text-gray-600 mt-2">Mã booking: #{booking.id}</p>
        </div>
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm">
              ...
              <div className="bg-blue-50 p-4 rounded-lg space-y-2">
                ...
                <div>
                  ...
                  <span className="font-bold text-blue-600 text-lg">{formatPrice(depositAmount)}</span>
                </div>
                <div>
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between py-2 border-b border-gray-200">
                  <span className="text-gray-600">Cụm sân</span>
                  <span className="font-semibold">{booking.complexName}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-gray-200">
                ...
                <div className="flex justify-between py-2 border-b border-gray-200">
                  ...
                  <span className="font-semibold">{formatDateTime(booking.bookingDate)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-gray-200">
                  <span className="text-gray-600">Giờ</span>
                  <span className="font-semibold">{booking.startTime?.substring(0, 5)} - {booking.endTime?.substring(0, 5)}</span>
                </div>
